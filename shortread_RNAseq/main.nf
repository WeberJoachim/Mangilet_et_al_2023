...
//for nucleotide frequency plots. This is ultra scuffed but it works for me. Probably using RSEQC would be best
...
workflow {
        ...
        //preparation for nuclotide frequency. The workflow extracts fasta files from the genomic positions, converts them to fastq with empty quality scores, since the downstream logic in R needs fastq as input
        ...
        //preprocess regions, and find overlapping features 
        ...
        py_extract_infos_motif(homer_count_coMotifs.out.info.splitText(by: 5, keepHeader: true), pyscript.collect())
